...
export default function ThirdPartyAppPage() {
    ...
    },
    {
      ...
      render: (_, record) => <>
        ...
        <Popconfirm
          ...
          onConfirm={() => {
            setLoading(true)
            openApi.thirdPartyApp.rollAccessSecret(record.id)
  ...
  const [dataSource, setDataSource] = useState(/** @type {import('@xezzon/zeroweb-sdk').ThirdPartyApp[]} */([]))
  const [loading, setLoading] = useState(false)
  const [count, setCount] = useState(0)
  const [record, setRecord] = useState(/** @type {import('@xezzon/zeroweb-sdk').ThirdPartyApp} */(null))
  const [accessSecret, setAccessSecret] = useState(/** @type {import('@xezzon/zeroweb-sdk').AccessSecret} */(null))
  ...
  useEffect(() => {
    ...
    openApi.thirdPartyApp
      .listMyThirdPartyApp({ top: pagination.pageSize, skip: (pagination.current - 1) * pagination.pageSize })
      .then(response => response.data)
      .then(({ content, page }) => {
  ...
  return <>
    ...
    >
      <Typography.Paragraph>
        ...
        <Typography.Text code copyable>{accessSecret?.accessKey}</Typography.Text>
      </Typography.Paragraph>
      <Typography.Paragraph>
        <Typography.Text>Secret Key: </Typography.Text>
        <Typography.Text code copyable>{accessSecret?.secretKey}</Typography.Text>
      </Typography.Paragraph>
