 ...
 * resources: import('@xezzon/zeroweb').MenuInfo[], 
 * modules: Record<string, () => Promise<{ default: React.ComponentType }>>,
 * rootRoutes: import('react-router').RouteObject[],
 * }} param0 
 * @returns 
 */
export default function ResourceContextProvider({ children, resources, modules, rootRoutes }) {
  ...
  const menus = useMemo(() => {
    const menus = resources
      .filter((resource) => [MenuType.ROUTE, MenuType.EXTERNAL_LINK, MenuType.EMBEDDED].includes(resource.type))
      .map((resource) => ({
        ...resource,
